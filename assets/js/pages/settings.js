            ...
            <div class="page">
                ...
                <div class="page-content">
                    <div class="form-section">
                        <h3>新規項目追加</h3>
                        <form id="add-item-form" class="form-row">
                            ...
                            <div class="form-group" style="flex: 1;">
                                <label>ウエイト(%)</label>
                                <input type="number" id="item-weight" required min="0" max="100">
                            </div>
                            <div class="form-group" style="flex: 1;">
                                <label>表示順</label>
                                <input type="number" id="item-order" required min="1">
                            </div>
                    ...
                    <div class="table-container">
                        <table class="table">
                            <thead><tr><th>表示順</th><th>項目名</th><th>説明</th><th>ウエイト(%)</th><th>操作</th></tr></thead>
                            <tbody>
                                ${items.map(item => `
                                    <tr>
                                        <td>${item.order}</td>
                                        ...
                                        <td>${item.weight || 0}%</td>
                                        <td>
                                            <button class="btn btn-danger" onclick="handleDeleteItem('${item.id}')">削除</button>
                                        </td>
                                    </tr>
                                `).join('')}
                            </tbody>
...
async function handleCreateItem(e) {
    ...
    if (!name || isNaN(weight) || isNaN(order)) {
        return showNotification('項目名、ウエイト、表示順は必須です', 'error');
    }

    try {
        await api.createEvaluationItem({ name, description, weight, order });
